...
产品保留一条可切换的识别链路：开启“精准识别”时，**豆包录音文件识别 2.0 优先，本地 Whisper 自动备用**；关闭时只走本地识别，不读取云端密钥、不上传录音。旧设置缺少该字段时默认开启，保持原有行为。
...
每次录音结束后会向 `recognition-history.jsonl` 追加一行 JSON：Windows 位于
`%LOCALAPPDATA%\CodexWhip\`，macOS 位于
`~/Library/Application Support/CodexWhip/`。记录包含录音时间与时长、结束原因、
识别设置、实际云端/本地尝试、豆包请求 ID 和状态码、回退原因、耗时、最终来源、
识别结果与文字。录音传输失败也会留下记录；录音未结束时无法形成完整识别记录。
记录只保存在本机，不含 API Key 或原始音频，但含识别文字，分享日志前请检查隐私。
旧版本未记录的识别来源无法从现有录音倒推。
...
界面的“录音识别增益”映射为 1–8 倍固定数字增益。增益依据录音峰值限制实际放大量，
